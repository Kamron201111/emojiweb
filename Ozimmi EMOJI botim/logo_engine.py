...
FONT_OPTIONS = {
    ...
    "poppins": ("Poppins", os.path.join(BASE_DIR, "fonts", "Poppins-Bold.ttf")),
}

PROFILE_FONT_OPTIONS = {
    "archivo": ("Archivo", os.path.join(BASE_DIR, "fonts", "Outfit-Bold.ttf")),
    "bebas": ("Bebas", os.path.join(BASE_DIR, "fonts", "LiberationSans-Bold.ttf")),
    "blackops": ("Black Ops", os.path.join(BASE_DIR, "fonts", "DejaVuSans-Bold.ttf")),
    "elsie": ("Elsie", os.path.join(BASE_DIR, "fonts", "IBMPlexSerif-Bold.ttf")),
    "jacquarda": ("Jacquarda", os.path.join(BASE_DIR, "fonts", "Poppins-Bold.ttf")),
    "betania": ("Betania", os.path.join(BASE_DIR, "fonts", "IBMPlexSerif-Bold.ttf")),
    "playwrite_br": ("Playwrite BR", os.path.join(BASE_DIR, "fonts", "JetBrainsMono-Bold.ttf")),
    "playwrite_de": ("Playwrite DE", os.path.join(BASE_DIR, "fonts", "Poppins-Bold.ttf")),
    "outfit": ("Outfit", os.path.join(BASE_DIR, "fonts", "Outfit-Bold.ttf")),
}
...
def profile_font_choice_kb(preview_text: str = ""):
    items = list(PROFILE_FONT_OPTIONS.items())
    rows = []
    for i in range(0, len(items), 2):
        pair = items[i:i + 2]
        row = []
        for key, (label, _path) in pair:
            display = f"{preview_text} ({label})" if preview_text else label
            row.append(InlineKeyboardButton(text=display, callback_data=f"pfont:{key}"))
        rows.append(row)
    rows.append([InlineKeyboardButton(text="Barcha Shriftlarda Yaratish", callback_data="pfont:all")])
    rows.append([InlineKeyboardButton(text="❌ Bekor qilish", callback_data="pfont:cancel")])
    return InlineKeyboardMarkup(inline_keyboard=rows)
...
def _retint_preserve_value(color_key, target_rgb, ref_key=None):
    ...
    faqat rang oilasi (hue/saturation) yangilanadi. Qora (#000000) va to'q
    ranglar uchun maxsus to'q ottenka hisoblanadi."""
    rgb = _approx_rgb_from_key(color_key) or (0.5, 0.5, 0.5)
    ref_rgb = _approx_rgb_from_key(ref_key) if ref_key else None
    ...
    if target_s < 0.1:  # Qora, Kulrang, Oq
        ...
        if target_v < 0.1:  # Nishon rang — Qora (#000000)
            # Asosiy fill = qora (0.0), highlight = to'q kulrang (0.15~0.25)
...
def _recolor_tgs_shapes_by_hue(shapes, outer_key, outer_rgb, inner_key, inner_rgb):
    ...
    yangi rangga o'tadi, faqat bittasi emas."""
    def visit(shape):
        ty = shape.get("ty")
        key = _shape_color_key(shape)
        if key is None:
        ...
        if ty in ("st", "gs"):
            if outer_key is not None:
                if key == outer_key:
                    _set_shape_color(shape, outer_rgb)
                elif _is_similar_hue(key, outer_key):
                    tint = _retint_preserve_value(key, outer_rgb, ref_key=outer_key)
                    _set_shape_color(shape, tint)
        elif ty in ("fl", "gf"):
            if inner_key is not None:
                if key == inner_key:
                    _set_shape_color(shape, inner_rgb)
                elif _is_similar_hue(key, inner_key):
                    tint = _retint_preserve_value(key, inner_rgb, ref_key=inner_key)
                    _set_shape_color(shape, tint)
...
def _normalize_lottie_keys(obj):
    """Lottie dict/list daraxti bo'ylab yurib, KERYL homoglif bilan
    yozilgan kalitlarni (masalan 'sh\u0430\u044es' = Cyrillic a/r)
    toza ASCII 'shapes' ga almashtiradi. Barcha dict kalitlari uchun
    NFKC normalizatsiyasi qo'llanadi."""
    if isinstance(obj, dict):
        ...
        for k, v in obj.items():
            if isinstance(k, str):
                ...
                if nk == "shapes":
                    k = "shapes"
            new_dict[k] = _normalize_lottie_keys(v)
        return new_dict
    elif isinstance(obj, list):
        return [_normalize_lottie_keys(x) for x in obj]
    else:
        return obj


def _project_mylogo_box_via_refid(data):
    """Agar placeholder bbox aniqlanmasa, refId='mylogo' layerning
    (position + scale) transformidan foydalanib, 512x512 mylogo asset
    lokal koordinatalaridan canvas koordinatalariga box ni proyeksiya
    qiladi. Muvaffaqiyatsiz bo'lsa None qaytaradi."""
    ref_layer = None
    ...
    def _get_ks_vec(key, default):
        v = ks.get(key, {}).get("k", default)
        if isinstance(v, list) and len(v) >= 2:
            return float(v[0]), float(v[1])
        return default

    px, py = _get_ks_vec("p", (None, None))
    sx, sy = _get_ks_vec("s", (100.0, 100.0))
    ax, ay = _get_ks_vec("a", (0.0, 0.0))
    ...
    pad = 0.14
    inner_l = local_l + asset_w * pad
    inner_t = local_t + asset_h * pad
    inner_r = local_r - asset_w * pad
    inner_b = local_b - asset_h * pad
    ...
    x0, y0 = project(inner_l, inner_t)
    x1, y1 = project(inner_r, inner_b)

    xmin, xmax = sorted([x0, x1])
    ymin, ymax = sorted([y0, y1])

    xmin = max(0.0, min(canvas_w, xmin))
    xmax = max(0.0, min(canvas_w, xmax))
    ymin = max(0.0, min(canvas_h, ymin))
    ymax = max(0.0, min(canvas_h, ymax))
...
def _ensure_mylogo_asset(data):
    """Agar shablonda 'mylogo' asset umuman bo'lmasa, uni yaratadi va
    ichiga bitta bo'sh placeholder (guruh) qo'yadi. Oxirida placeholder
    guruhini qaytaradi (yangi yaratilgan yoki avvalgi topilgan)."""
    canvas_w = data.get("w", 512)
    canvas_h = data.get("h", 512)
    ip = data.get("ip", 0)
    op = data.get("op", 180)
    ...
    placeholder_group = {
        "ty": "gr", "it": [], "nm": "Svg Group 0", "np": 0,
        "cix": 2, "bm": 0, "ix": 1, "mn": "ADBE Vector Group", "hd": False,
    }

    for asset in data.get("assets", []):
        if asset.get("id") == "mylogo":
            found = _find_mylogo_placeholder(data)
            if found is not None:
                return found
            if not asset.get("layers"):
                asset["layers"] = []
            asset["layers"].append({
                "ddd": 0, "ind": 1, "ty": 4, "nm": "Logo Layer", "sr": 1,
                "ks": {
                    "a": {"a": 0, "k": [0, 0]},
                    "p": {"a": 0, "k": [0, 0]},
                    "s": {"a": 0, "k": [100, 100]},
    ...
    for layer in data.get("layers", []) or []:
        if layer.get("refId") == "mylogo":
            ...
            break
    if not has_ref:
        data.setdefault("layers", []).insert(0, {
            "ddd": 0, "ind": 999000, "ty": 0, "nm": "Logo Precomp", "sr": 1,
            "refId": "mylogo",
            "ks": {
                "a": {"a": 0, "k": [0, 0]},
...
def _guess_default_logo_box(data, dir_path=None):
    """Agar placeholder box'ini aniqlab bo'lmasa, shablon turiga qarab
    standart logo joyini (canvas markazida) taxminiy qaytaradi."""
    canvas_w = data.get("w", 512)
    canvas_h = data.get("h", 512)
    ...
    if "tgs2" in folder or "tgs3" in folder:
        pad = 0.30
    else:
        pad = 0.22

    cx, cy = canvas_w / 2, canvas_h / 2
    half_w = canvas_w * (0.5 - pad)
    half_h = canvas_h * (0.5 - pad)
...
def build_tgs_sticker(template_filename, svg_text, outer_hex, inner_hex, logo_hex, size_percent=100, watermark=False, dir_path=None):
    ...
    with open(path, "r", encoding="utf-8") as f:
        data = json.load(f)
    data = copy.deepcopy(data)
    data = _normalize_lottie_keys(data)

    folder = os.path.basename(os.path.normpath(d)).lower()
    is_extra_section = ("tgs2" in folder) or ("tgs3" in folder)
    ...
    for shapes in main_shape_lists:
        _recolor_tgs_shapes_by_hue(shapes, dominant_stroke, outer_rgb, dominant_fill, inner_rgb)
    ...
    if logo_group is None:
        logo_group = _ensure_mylogo_asset(data)

    if logo_group is not None and svg_text:
        svg_subpaths, _ = svg_parse_paths(svg_text)
        box = _bbox_of_shape_group(logo_group)
        if box is None:
            box = _project_mylogo_box_via_refid(data)
        if box is None:
            box = _guess_default_logo_box(data, dir_path=d)

        effective_percent = size_percent
        if is_extra_section and effective_percent >= 125:
            effective_percent = int(effective_percent * 0.82)

        extra_scale = max(SIZE_MIN_PERCENT, min(SIZE_MAX_PERCENT, effective_percent)) / 100.0
