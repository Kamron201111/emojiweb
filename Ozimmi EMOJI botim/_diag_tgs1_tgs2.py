...
print("=== templates_tgs (1-bolim) folderi - random 3 ta sample ===")
for folder in ['templates_tgs', 'templates_tgs2']:
    print(f"\n--- {folder} ---")
    ...
    for fname in samples:
        ...
        bbox = logo_engine._bbox_of_shape_group(lg) if lg else None
        if bbox and ref_p:
            ...
            print(f"  {fname}: ref_p={ref_p} ref_s={ref_s} ref_a={ref_a} bbox_center=({lcx:.0f},{lcy:.0f}) CANVAS_center=({ccx:.0f},{ccy:.0f}) {cw:.0f}x{ch:.0f}")
